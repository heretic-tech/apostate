...
A failed or partial GeoIP lookup invents nothing. The field it could not
answer for falls to the host and the launcher warns. A locale is not looked
up at all -- no provider returns one -- it is inferred from the exit country
through `config/country-locales.json`, which both packages ship and which
names all 257 ISO-3166 territories, so the only lookup that resolves no
locale is one that returned no country. An Albanian exit resolves
`Europe/Tirane` and `sq`. The table is generated by
`scripts/generate-country-locales.py` from CLDR territoryInfo and Chromium's
own `kAcceptLanguageList`; it replaced a hand-kept list of 45 countries that
left most of the world's exits with no language at all.
